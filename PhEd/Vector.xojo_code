	...
	#tag Method, Flags = &h0
		Sub Draw(g as Graphics, X as Double, Y as Double)
		  ...
		  For i As Integer = 0 To elements.Ubound
		    elements(elements.Ubound-i).Draw(g, X + 3, newY) // 3 is a good baseline offset, the parenthesis might grow - needs work
		    newY = newY - elements(elements.Ubound-i).Height(g) + 2
		    If elements(elements.Ubound-i).Width(g) > widest Then
		      widest = elements(elements.Ubound-i).Width(g)
		    End If
		  Next
		  ...
		  if elements.Ubound < 3 then
		    ...
		    Dim lParen As New Bracket(g,false,Y - newY)
		    lParen.Draw(g, X, Y)
		    
		    Dim rParen As New Bracket(g,true,Y - newY)
		    rParen.Draw(g, X + lParen.GetWidth + widest, Y)
		  end
